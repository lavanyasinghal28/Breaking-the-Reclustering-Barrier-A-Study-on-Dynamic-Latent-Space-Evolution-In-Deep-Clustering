...
def plot_comparison(results, reset_interval):
    ...
    max_ep = max(results['DCN + BRB']['epoch'])
    ...
    fig.suptitle("Demonstrating the Reclustering Barrier on MNIST (DCN)",
                 fontsize=15, fontweight='bold', y=1.02)
    fig.tight_layout()
    _save(fig, "comparison_all.png")
...
def plot_accuracy_only_comparison(results, reset_interval):
    ...
    for label, history in results.items():
        ax.plot(history['epoch'],
                ...
                label=label, color=COLORS[label], linewidth=2.5)
    _add_intervention_lines(ax, max(results['DCN + BRB']['epoch']), reset_interval)
