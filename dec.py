...
def train_dec_with_brb(
    ...
    """Train the DEC model with BRB."""
    optimizer = torch.optim.Adam(dec_model.parameters(), lr=1e-3)
    mse_loss_fn = nn.MSELoss()

    dec_model.init_kmeans(train_loader, device)
    ...
    for epoch in range(1, epochs + 1):
        if epoch > 1 and epoch % reset_interval == 0:
            ...
            apply_soft_reset_to_network(dec_model.autoencoder, alpha=alpha)
            ...
            optimizer.state[dec_model.centers] = {}
        ...
        for batch_x, _ in train_loader:
            ...
            z, x_recon = dec_model(batch_x)
            loss_recon = mse_loss_fn(x_recon, batch_x)

            distances = torch.cdist(z, dec_model.centers)
            assigned_centers_idx = torch.argmin(distances, dim=1)
            loss_cluster = mse_loss_fn(z, dec_model.centers[assigned_centers_idx])
        ...
        with torch.no_grad():
            for batch_x, batch_y in test_loader:
                batch_x = batch_x.to(device)
                z = dec_model.autoencoder.encode(batch_x)
                distances = torch.cdist(z, dec_model.centers)
